...
R"""
    ...
    mle<-mle.vam(System&Time&Type~(ARAInf(0.4) | Weibull(0.001,1.5| 1.3*cov1 + -2.2*cov2)),data=dataDF,data.covariates = dataCov)
...
R"""
    ...
    dataDF<-data.frame(System=c(rep(1,5)),Time=c(0.800,2.646,3.190,3.916,4.109),Type=rep(-1,5))
    dataCov<-data.frame(cov1=4.336)
    mle<-mle.vam(System&Time&Type~(ARA1(0.4) | Weibull(0.001,1.5| 1.3*cov1)),data=dataDF,data.covariates = dataCov)
...
dataDF= @rget dataDF
θ =  @rget theta
m = @vam( System & Time & Type ~ (ARA1(0.4) | Weibull(0.001,1.5| 1.3*cov1)), data=dataDF, datacov=dataCov)
lnL = res[:lnL]
dlnL = res[:dlnL]
d2lnL = res[:d2lnL]
contrast(m, θ, profile=false)-lnL
